...
function renderJobs() {
  ...
  jobList.innerHTML = filtered.length ? filtered.map(job => `<article class="job-card"><div class="job-logo" style="background:${job.color}">${job.logo}</div><div><h3>${job.title}</h3><p>${job.company} · ${job.location}</p><div class="tags"><span class="tag">${job.type}</span>${job.visa ? '<span class="tag">Visa support</span>' : ''}</div></div><div class="job-meta"><b>${job.category}</b><button aria-label="View ${job.title}">→</button></div></article>`).join('') : '<div class="empty">No roles match those filters just yet. Try broadening your search.</div>';
}
